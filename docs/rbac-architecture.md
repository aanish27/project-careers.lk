...
The permission registry (`packages/types/permissions.ts`) is deliberately **code, not data**: a permission that no route actually checks is meaningless, and routes are defined in code. The `permissions` table is a synced _projection_ of that file — permissions are never created through the API (see [`PermissionsController`](../apps/api/src/modules/rbac/permissions.controller.ts), which is read-only by design).

## 3. Components and responsibilities

| Component                                        | Lives in                                                                                                                                                                                                                                                                                                             | Responsibility                                                                                                                                                                                                                                                                                         |
| ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Permission registry                              | [`packages/types/permissions.ts`](../packages/types/permissions.ts)                                                                                                                                                                                                                                                  | `PERMISSION_DESCRIPTIONS` (key → human description), `PermissionKey` type, `PERMISSIONS` constants, `SUPER_ADMIN_ROLE_SLUG`. The only place a permission is _defined_.                                                                                                                                 |
| Shared entity shapes                             | [`packages/types/rbac-entities.ts`](../packages/types/rbac-entities.ts)                                                                                                                                                                                                                                              | `AdminUser`, `Role`, `PermissionCatalogEntry`, `PermissionGroup` — the wire shapes both API and web agree on.                                                                                                                                                                                          |
...
## 10. `@careerslk/types` is split to keep Node-only code out of the browser bundle

`packages/types` has two build entries: `index.ts` (client-safe — enums, constants, the permission registry, RBAC entity shapes) and `ssrf.ts` (`assertNotSsrf`, which needs `node:dns`/`node:url`, used only by `CompaniesService` and the scraper's `playwright.service.ts`). They're published as separate subpaths — `@careerslk/types` and `@careerslk/types/ssrf`.

This split exists _because of_ RBAC: before this feature, no client component imported a **runtime value** from `@careerslk/types` (only types, which get elided from the bundle at compile time). `<Can permission={PERMISSIONS.USERS_CREATE}>` and friends import `PERMISSIONS` as an actual object, which pulls in the whole `index.ts` barrel at runtime — and until this split, that barrel re-exported the SSRF helper too, breaking the browser build (`Cannot find module 'node:dns'`).

**The rule going forward:** anything added to `packages/types/index.ts` (or files it re-exports) must be safe to run in a browser. If you need a Node-only utility shared between `apps/api` and `apps/scrapper`, give it its own entry (see `ssrf.ts` for the pattern) and its own `exports` subpath in `packages/types/package.json`+`tsdown.config.ts` — never fold it into `index.ts`.
