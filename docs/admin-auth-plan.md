...
**Refresh strategy — proactive, not reactive, and why:** given this is a low-QPS internal admin tool with no proxied data-fetch routes yet in this scope, there's no call site for "retry after a 401" to attach to — the only place an access token is used at all is inside the session-reading function itself. So: check `accessTokenExpiresAt` against a small buffer (e.g. 60s) every time the session is read, and refresh transparently before returning if it's stale or about to be. Wrap this reader in React's `cache()` so it only actually runs (and only actually refreshes) once per request/render pass, even if multiple Server Components on the same page ask for the session — this matters because the backend **rotates and invalidates** the previous refresh token on every use, so two near-simultaneous refresh calls using the same stale token would race and the loser would get logged out.
...
5. `getSession = cache(async (): Promise<SessionPayload | null> => { ... })` — the DAL-style reader (matches the pattern in the Next.js authentication guide's `verifySession()` example, adapted here to also own the proactive refresh):
   - Read + verify the cookie via `decryptSession`. If `null` → return `null`.
   - If `accessTokenExpiresAt` is within `REFRESH_THRESHOLD_MS` of now (or already past) → call `refreshRequest(session.refreshToken)`; on success, `updateSessionTokens(...)` and use the merged result; on failure (refresh token itself dead) → `destroySession()` and return `null`.
   - Return the (possibly refreshed) payload.
   - **Why `cache()` matters here specifically:** without it, if both `admin/layout.tsx` (Step 10) and `admin/page.tsx` (Step 11) call this on the same request, you'd fire two refresh calls, and the second one would 401 against the now-rotated-and-invalidated token from the first — silently logging the user out on every page load. `cache()` memoizes per-request, so this can't happen.
6. `verifySession = cache(async (): Promise<SessionPayload> => { const s = await getSession(); if (!s) redirect('/admin/login'); return s })` — the redirecting variant, for use in pages/Server Actions.
...
**Why `getSession()` and `verifySession()` need to be separate functions, not one:** `proxy.ts` (Step 9) needs the non-network, non-redirecting signature-check-only behavior — it must never make an outbound refresh call on every navigation (the Next docs are explicit that proxy runs on every request including prefetches, and should stay fast/local). Pages need the redirecting behavior. Conflating them would force proxy to either skip the refresh check (fine) or make it slow and network-bound (not fine) — keep them cleanly separated: proxy uses `decryptSession()` directly (Step 9), pages use `verifySession()`/`requireRole()`.
...
- `logout(): Promise<void>`: reads the current session via `getSession()`, if present calls `logoutRequest(session.accessToken)` wrapped in a `.catch(() => {})` (best-effort — a dead/expired access token shouldn't block the user from logging out locally), then unconditionally `destroySession()` and `redirect('/admin/login')`.
...
**Why this must stay network-free** (no calls to `getSession()`'s refresh logic): proxy runs on every navigation to a matched path, including prefetches the browser fires speculatively — the Next.js docs are explicit that proxy "is not intended for slow data fetching" and optimistic checks should read only the cookie. The _real_ enforcement (including the token-freshness/refresh concern) belongs in `verifySession()` at the page level (Step 11), which only actually runs when a page genuinely renders.
...
- **Refresh token itself expired/invalid:** `getSession()` (Step 6.5) catches the `refreshRequest` failure, destroys the cookie, returns `null`; the calling `verifySession()` then redirects to login exactly like any unauthenticated case — no special-case messaging needed.
...
- **Concurrent tabs racing a refresh:** accepted limitation for a low-traffic internal tool — the backend's refresh-token rotation means a losing concurrent refresh 401s and that tab gets logged out. `cache()`-scoping (Step 6.5) keeps this to at most once _per request_, but doesn't prevent two genuinely simultaneous requests (e.g. two different tabs) from racing.
...
8. Temporarily lower `ACCESS_TOKEN_TTL_MS` (frontend) and the backend's `JWT_ACCESS_TOKEN_EXPIRES_IN` env var to something like `10s` for a manual test, log in, wait past that window, then reload `/admin` → confirm the page still renders normally (the proactive refresh in `getSession()` happened transparently) without forcing a re-login. Revert both values afterward.
